...
local function connectPetEffectClient()
	if _G.__PetEffectsClientConnected then return end
	local effectModuleScript = Modules:FindFirstChild("EffectModule") or Modules:WaitForChild("EffectModule", 10) or ReplicatedStorage:FindFirstChild("EffectModule")
	local effectRemote = ReplicatedStorage:FindFirstChild("PetEffectEvent") or ReplicatedStorage:WaitForChild("PetEffectEvent", 10)
	if not effectModuleScript or not effectRemote or not effectRemote:IsA("RemoteEvent") then
		return
	end
...
task.defer(connectPetEffectClient)
